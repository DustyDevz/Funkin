...
namespace Funkin::Platform::Input {
    ...
    void pollXInput(InputRingBuffer<4096>& ring,
        ...
        for (DWORD i = 0; i < 4; ++i) {
            ...
            checkBtn(XINPUT_GAMEPAD_LEFT_SHOULDER,  ControllerButton::LBumper);
            checkBtn(XINPUT_GAMEPAD_RIGHT_SHOULDER, ControllerButton::RBumper);
            checkBtn(XINPUT_GAMEPAD_LEFT_THUMB,     ControllerButton::LStick);
            checkBtn(XINPUT_GAMEPAD_RIGHT_THUMB,    ControllerButton::RStick);
            ...
            if (curr.sThumbLX != prev.sThumbLX)
                pushAxis(ControllerAxis::LX, norm16(curr.sThumbLX));
            if (curr.sThumbLY != prev.sThumbLY)
                pushAxis(ControllerAxis::LY, -norm16(curr.sThumbLY));
            if (curr.sThumbRX != prev.sThumbRX)
                pushAxis(ControllerAxis::RX, norm16(curr.sThumbRX));
            if (curr.sThumbRY != prev.sThumbRY)
                pushAxis(ControllerAxis::RY, -norm16(curr.sThumbRY));
            if (curr.bLeftTrigger  != prev.bLeftTrigger)
                pushAxis(ControllerAxis::LT, norm8(curr.bLeftTrigger));
            if (curr.bRightTrigger != prev.bRightTrigger)
                pushAxis(ControllerAxis::RT, norm8(curr.bRightTrigger));
