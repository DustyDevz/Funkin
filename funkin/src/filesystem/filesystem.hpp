...
namespace Funkin {
    ...
    public:
        ...
        static bool isEngineResource(const std::string& virtualPath);

        static std::vector<uint8_t> readBytes(const std::string& virtualPath);
    ...
    private:
        ...
        static std::vector<uint8_t> readBytesFromQtResource(const std::string& qtPath);
    };
}