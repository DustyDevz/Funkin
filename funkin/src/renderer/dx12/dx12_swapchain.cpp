...
namespace Funkin::Renderer::DX12 {
    void DX12Swapchain::init(ID3D12Device* device, IDXGIFactory6* factory,
        ...
        auto hwnd = Funkin::Platform::Window_Win32::get().hwnd();

        ComPtr<IDXGISwapChain1> sc1;
        if (FAILED(factory->CreateSwapChainForHwnd(queue, hwnd, &desc, nullptr, nullptr, &sc1)))
    ...
    void DX12Swapchain::resize(ID3D12Device* device, int w, int h) {
        for (auto& rt : m_rts) rt.Reset();

        if (FAILED(m_swapchain->ResizeBuffers(FRAME_COUNT, (UINT)w, (UINT)h, DXGI_FORMAT_UNKNOWN, 0)))
