...
namespace Funkin::Renderer::GAL {
    void DX12Gal::beginRenderPass(const RenderPassDesc& desc) {
        ...
        if (desc.useSwapchainTarget) {
            ...
            {
                const Math::Color& cc = desc.colorAttachments[0].clearColor;
            ...
            for (uint32_t i = 0; i < desc.colorCount; ++i) {
                ...
                if (desc.colorAttachments[i].loadOp == LoadOp::Clear) {
                    const Math::Color& cc = desc.colorAttachments[i].clearColor;
                    FLOAT clearColor[4] = { cc.r, cc.g, cc.b, cc.a };
    ...
    void DX12Gal::setScissor(const Math::Rect& rect) {
        D3D12_RECT scissor = {
    ...
    void DX12Gal::drawLine(Math::Vec2, Math::Vec2, Math::Color, float)        {}
    void DX12Gal::drawRect(Math::Rect, Math::Color, float)              {}
    void DX12Gal::drawFilledRect(Math::Rect, Math::Color)               {}
    void DX12Gal::drawCircle(Math::Vec2, float, Math::Color, int)       {}

    Math::Vec2 DX12Gal::project(Math::Vec3 worldPos, const Math::Mat4& viewProj) const {
        Math::Vec4 clip = viewProj * Math::Vec4(worldPos, 1.0f);
        Math::Vec3 ndc  = clip.xyz() / clip.w;
        return {
