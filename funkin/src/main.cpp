...
static int run() {
    ...
    const auto* vs = shaderLoader.get("test", Shader::ShaderStage::Vertex);
    const auto* ps = shaderLoader.get("test", Shader::ShaderStage::Pixel);
    ...
    auto vsHandle = gal->createShader({ vs->bytecode.data(), vs->bytecode.size(), GAL::ShaderStage::Vertex });
    auto psHandle = gal->createShader({ ps->bytecode.data(), ps->bytecode.size(), GAL::ShaderStage::Pixel  });

    LOG_REND("init: test component");
    Funkin::Scene::Components::Test test;
    test.init(gal, vsHandle, psHandle);
    ...
    gal->waitIdle();
    test.shutdown(gal);
    gal->destroyShader(vsHandle);
    gal->destroyShader(psHandle);
